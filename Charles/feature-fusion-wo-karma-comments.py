...
# (title_token_ids, type_id, day_of_week_id, domain_id, hour_of_day, score, seq_length)
...
def fusion_collate_fn(batch):
    token_lists, type_ids, day_ids, domain_ids, hours, scores, lengths = zip(*batch)
    ...
    return padded_tokens, type_ids, day_ids, domain_ids, hours, scores, lengths
...
class FusionModel(nn.Module):
    def __init__(self, word2vec_weight, num_words, 
        ...
        fused_dim = EMBEDDING_DIM + TYPE_EMB_DIM + DAY_EMB_DIM + DOMAIN_EMB_DIM + 1  # Only 1 continuous feature (hour)
    ...
    def forward(self, title_tokens, lengths, type_ids, day_ids, domain_ids, hours):
        ...
        cont_features = hours.unsqueeze(1)  # shape (B, 1)
...
def train_fusion(model, dataloader, device, optimizer, criterion, wandb_run=None):
    ...
    for batch in tqdm(dataloader, desc="Training"):
        title_tokens, type_ids, day_ids, domain_ids, hours, scores, lengths = [b.to(device) for b in batch]
        optimizer.zero_grad()
        outputs = model(title_tokens, lengths, type_ids, day_ids, domain_ids, hours)
