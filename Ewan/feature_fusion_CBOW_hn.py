...
class HNRegressionDataset(Dataset):
    def __init__(self, parquet_path, word_embedder, max_title_tokens=MAX_TITLE_TOKENS):
...
def train_and_eval():
    ...
    for epoch in range(1, config.epochs + 1):
        model.train()
        train_losses = []
        for token_ids, features, targets in tqdm(train_dl, desc=f"Epoch {epoch} [train]"):
        ...
        val_losses = []
        with torch.no_grad():
            for token_ids, features, targets in tqdm(val_dl, desc=f"Epoch {epoch} [val]"):
                token_ids = token_ids.to(device)
                features = {k: v.to(device) for k, v in features.items()}
                targets = targets.to(device)
                preds = model(token_ids, features)
